                    ...
                    Ok(_) => {
                        ...
                        if current_slot == req_shared.with_tag(1) {
                            ...
                            match slot_ref.compare_exchange(
                                ...
                                Err(_) => Ok(()), /* A pop thread matched at the very last
                                                   * millisecond (tag became 3) */
                            }
                        } else {
                            // The slot changed. It's either our request marked as acknowledged (tag
                            // 3) or it was already cleared and reused
                            // by another thread. Only clear it if it
                            // explicitly belongs to our transaction.
                            if current_slot == req_shared.with_tag(3) {
                                let _ = slot_ref.compare_exchange(
                                    req_shared.with_tag(3),
                                    Shared::null(),
                                    Ordering::Release,
                                    Ordering::Relaxed,
                                    guard,
